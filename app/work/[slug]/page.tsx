  ...
  return (
    <div className="pt-32 pb-24 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Tech Stack & Tools */}
        <section className="mb-12 p-6 rounded-2xl bg-zinc-50 border border-zinc-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {project.techStack.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-slate-900 mb-3">Tech Stack</h3>
                <div className="flex flex-wrap gap-2">
                  {project.techStack.map((tech) => (
                    <span
                      key={tech}
                      className="px-3 py-1.5 text-sm bg-white text-slate-700 rounded-lg border border-zinc-200"
                    >
                      {tech}
                    </span>
                  ))}
            ...
            {project.tools.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-slate-900 mb-3">Tools</h3>
                <div className="flex flex-wrap gap-2">
        ...
        <div className="space-y-12">
          ...
          {project.outcome && (
            <section>
              <h2 className="text-2xl font-semibold text-slate-900 mb-4">The Outcome</h2>
              <p className="text-slate-600 leading-relaxed mb-6">{project.outcome}</p>
              ...
              {project.metrics && project.metrics.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {project.metrics.map((metric, index) => (
                    <div
                      key={index}
                      className="p-4 rounded-xl bg-zinc-50 border border-zinc-200 text-center"
                    >
                      <span className="text-lg font-semibold text-slate-900">{metric}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="mt-16 pt-8 border-t border-zinc-200">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-slate-500">Interested in working together?</p>
            <a
              href="/#contact"
